...
(() => {
  ...
  // Mirrors the app's useVoiceConfig storage shape.
  ...
  // Mirrors the app's voiceApi.ts: direct OpenAI-compatible endpoint when a
  // baseUrl is configured, otherwise the Express proxy with the session JWT.
  ...
  // Insert at the caret in a React-controlled textarea. execCommand keeps the
  // undo stack and fires the input event React listens to; the native-setter
  // fallback covers engines without execCommand.
  ...
  function onKeyDown(e) {
    ...
    if (recording) {
      // Held through the session: swallow repeats and extra taps.
      e.preventDefault();
      e.stopPropagation();
      return;
    }
    ...
    // First press: take over the space character and decide by hold duration.
    holding = true;
    e.preventDefault();
    e.stopPropagation();
  ...
  function onKeyUp(e) {
    ...
    holding = false;
    if (pressTimer) {
      // Released before the threshold: it was a normal space keystroke.
      clearTimeout(pressTimer);
      pressTimer = null;
