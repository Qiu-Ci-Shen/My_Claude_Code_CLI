/**
 * rewind.service 纯函数单测（tsx --test，随 npm test 运行）
 * 用例自 plugins/claude-rewind/tests/server.test.js 全量迁入。
 *
 * 覆盖：
 ...
 *     相对路径 cwd 解析、未知目标
 *
 * 快照语义见插件 docs/修复指南.md（2026-08-26 真实转录逆向，git 历史可查）：
...
import { normalizeForMatch, locateTargetMessage, buildRestorePlan } from '../rewind.service.js';

const CWD = 'D:/work';
const CHK = 'C:/checkpoints';
const p = (rel: string) => path.join(CWD, rel);
const bk = (name: string) => path.join(CHK, name);

// ---- 构造器 ----------------------------------------------------------------

function user(uuid: string, parentUuid: string | null, timestamp: string, content: unknown, extra: Record<string, unknown> = {}) {
  return { type: 'user', uuid, parentUuid, timestamp, message: { content }, ...extra };
}

function snapshot(uuid: string, parentUuid: string | null, trackedFileBackups: Record<string, { backupFileName?: string | null }>, extra: Record<string, unknown> = {}) {
  return { type: 'file-history-snapshot', uuid, parentUuid, snapshot: { trackedFileBackups }, ...extra };
}

function delta(uuid: string, parentUuid: string | null, trackingPath: string, backupFileName: string | null, extra: Record<string, unknown> = {}) {
  const entry: Record<string, unknown> = { type: 'file-history-delta', uuid, parentUuid, trackingPath, ...extra };
...
test('locate: 时间戳+归一化前缀命中（markdown 渲染差异）', () => {
  ...
  const hit = locateTargetMessage(entries as never, {
...
test('locate: isSidechain 的用户消息被跳过', () => {
  ...
  const hit = locateTargetMessage(entries as never, { timestamp: '2026-08-27T10:00:00.000Z', textPrefix: 'hello' });
  assert.equal(hit?.uuid, 'u1');
});
...
test('locate: 无匹配返回 null', () => {
  ...
  assert.equal(locateTargetMessage(entries as never, { timestamp: '2099-01-01T00:00:00.000Z', textPrefix: 'hello' }), null);
});
...
test('plan: 回合内快照优先于前置快照', () => {
  ...
  const plan = buildRestorePlan(entries as never, 'u1', CHK, { cwd: CWD });
...
test('plan: 未知目标 uuid → 空计划', () => {
  const entries = [user('u1', null, '2026-08-27T10:00:00.000Z', 'hello world')];
  assert.deepEqual(buildRestorePlan(entries as never, 'nope', CHK, { cwd: CWD }), { restore: [], remove: [] });
});
