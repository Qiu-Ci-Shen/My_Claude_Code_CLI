...
export default function SidebarSessionItem({
  ...
  const copyLabel = copyState === 'loading'
    ? t('sessions.copySessionIdLoading', { provider: providerLabel })
    : copyState === 'copied'
      ? t('sessions.copySessionIdCopied', { provider: providerLabel })
      : copyState === 'error'
        ? providerSessionId
          ? t('sessions.copySessionIdFailed', { provider: providerLabel })
          : t('sessions.copySessionIdUnavailable', { provider: providerLabel })
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    {copyState === 'error' && (
                      <span className="mt-0.5 block text-xs">{t('sessions.tapToRetry')}</span>
                    )}
                  </span>
                </button>
      ...
      <div className="hidden md:block">
        <a
          ...
          className={cn(
            ...
            'h-auto w-full justify-start rounded-md p-2 pr-11 text-left font-normal transition-all duration-150',
            isSelected
              ? 'bg-primary/10'
              : isProcessing
                ? 'hover:bg-muted/25'
              ...
              <ActionMenu
                ...
                items={[
                  ...
                  },
                  {
                    key: 'copy',
                    label: copyLabel,
                    description: copyState === 'error' ? t('sessions.clickToRetry') : undefined,
                    icon: CopyStateIcon,
